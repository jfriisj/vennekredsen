...
function showDashboard() {
    ...
    document.getElementById("resource-dashboard").hidden = false;
...
async function showResourceView(viewId) {
    ...
    document.getElementById("resource-dashboard").hidden = true;
    document.querySelectorAll("[data-resource-view]").forEach(view => {
...
function renderApplications() {
    ...
    visible.forEach(application => {
        ...
        select.addEventListener("change", async () => {
            try {
                await apiRequest(
                    `/api/member/applications/${application.id}/status`,
                    {
                        method: "PUT",
        ...
        if (application.status === "rejected") {
            ...
            deleteButton.addEventListener("click", async () => {
                ...
                try {
                    await apiRequest(
                        `/api/member/applications/${application.id}`,
                        {
                            method: "DELETE",
                        }
                    );
