...
async function loadNextEvent () {
  ...
  try {
    ...
    if (!nextEvent) {
      ...
      eventStatus.textContent = 'Hold øje med siden for næste arrangement.'
      return
    }

    eventTitle.textContent = eventNames[nextEvent.key] || 'Kommende arrangement'
...
function createProjectCard (project) {
  ...
  amount.textContent = `${Number(project.belob || 0).toLocaleString('da-DK')} kr.`
  ...
  meta.textContent = project.godkendt_dato
    ? `Godkendt ${new Intl.DateTimeFormat('da-DK', { dateStyle: 'medium' }).format(
