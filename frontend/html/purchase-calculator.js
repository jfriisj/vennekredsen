...
function getFilteredItems() {
  ...
  return allItems.filter(item => {
    const storeMatches = !selectedStore || item.indkobssted === selectedStore;
    const categoryMatches = !selectedCategory || item.kategori === selectedCategory;
...
async function loadSpreadsheet(file) {
  ...
  try {
    ...
    if (allItems.length === 0) {
      throw new Error("Arket indeholder ingen genkendelige indkøbsvarer.");
    }
    ...
    renderItems();
    setStatus(`${allItems.length} varer indlæst fra ${file.name}.`, "success");
  } catch (error) {
    ...
    resultsTable.innerHTML =
      '<tr><td colspan="9" class="empty-state"></td></tr>';
    resultsTable.querySelector("td").textContent = error.message;
...
function safeSheetName(value, fallback) {
  const cleaned = String(value).replace(/[\\/:*?"<>|]/g, "").slice(0, 31);
  return cleaned || fallback;
}

function downloadByStore() {
  const workbook = XLSX.utils.book_new();
  uniqueSorted(allItems.map(item => item.indkobssted)).forEach((store, index) => {
...
function downloadByCategory() {
  const workbook = XLSX.utils.book_new();
  uniqueSorted(allItems.map(item => item.kategori)).forEach((category, index) => {
